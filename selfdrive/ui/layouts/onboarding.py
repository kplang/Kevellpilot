...
class TermsPage(Widget):
  def __init__(self, on_accept=None, on_decline=None):
    ...
    self._title = Label(tr("KEVELL MOTORS"), font_size=90, font_weight=FontWeight.BOLD, text_alignment=rl.GuiTextAlignment.TEXT_ALIGN_LEFT)
    self._desc = Label(tr("A Companion\n\nYou must accept the Terms and Conditions to use KevellPilot. Read the latest terms at https://comma.ai/terms before continuing."),
...
class DeclinePage(Widget):
  def __init__(self, back_callback=None):
    super().__init__()
    self._text = Label(tr("You must accept the Terms and Conditions in order to use KevellPilot."),
                       font_size=90, font_weight=FontWeight.MEDIUM, text_alignment=rl.GuiTextAlignment.TEXT_ALIGN_LEFT)
    self._back_btn = Button(tr("Back"), click_callback=back_callback)
    self._uninstall_btn = Button(tr("Decline, uninstall KevellPilot"), button_style=ButtonStyle.DANGER,
